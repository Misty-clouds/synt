  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-app-border px-5 py-3">
        ...
        <p className="text-xs text-zinc-500">{hypotheses.length} under test</p>
      </div>
      <div className="flex-1 space-y-2.5 overflow-y-auto p-3">
        {hypotheses.length === 0 && (
          <p className="px-2 py-4 text-xs text-zinc-600">Forming hypotheses…</p>
        )}
        {hypotheses.map((h) => {
            ...
            h.status === 'confirmed'
              ...
              : h.status === 'rejected'
                ...
                : h.status === 'testing'
                  ...
                  : 'border-app-border';
          return (
            <div key={h.id} className={cx('rounded-xl border bg-app-card-alt p-3', tone)}>
